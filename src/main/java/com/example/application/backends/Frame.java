...
public class Frame {
    ...
    private void convertMainLinearlyDistributedLoadsToSubLinearlyDistributedLoads () {
        for (LinearlyDistributedLoad ldl : mainLinearlyDistributedLoads) {
            ...
            for (short i = subFrameMembersStartingIndex.get(ldl.MN); i < subFrameMembersStartingIndex.get(ldl.MN + 1); i++) {
                ...
                if (BCx.compareTo(subFrameMembers.get(i).BCx) == 0) {
                    BMN = i;
                }
                if (ECx.compareTo(subFrameMembers.get(i).ECx) == 0) {
                    EMN = i;
                    break;
                }
            }
    ...
    private void calSubFrameMembersStiffness () {
        for (FrameMember sfm : subFrameMembers) {
            sfm.BJFR[0] = subJoints.get(sfm.BJN).FR[0];
            sfm.BJFR[1] = subJoints.get(sfm.BJN).FR[1];
            sfm.BJFR[2] = subJoints.get(sfm.BJN).FR[2];
            sfm.EJFR[0] = subJoints.get(sfm.EJN).FR[0];
            sfm.EJFR[1] = subJoints.get(sfm.EJN).FR[1];
            sfm.EJFR[2] = subJoints.get(sfm.EJN).FR[2];
            //modification (eg, end hinges, etc.)
            sfm.calStiffness();
    ...
    private void checkIfStable () {
        ...
        for (int i = 0; i < O.row; i ++) {
            if (!isWithinTolerance(O.data[i][0], BigDecimal.ZERO, new BigDecimal("0.01"))) {
                isStable = false;
                break;
            }
        }
