...
public class FrameMember {
    ...
    public FrameMember (BigDecimal BCx, BigDecimal BCy, BigDecimal ECx, BigDecimal ECy, CrossSection CS) {
        ...
        this.E = CS.E;
        this.I = CS.I;
        L_15 = (ECx.subtract(BCx).pow(2).add(ECy.subtract(BCy).pow(2))).sqrt(new MathContext(precision)).setScale(15, RoundingMode.HALF_UP).stripTrailingZeros();
        L = L_15.setScale(2, RoundingMode.FLOOR).stripTrailingZeros();
        cos = ECx.subtract(BCx).divide(L_15, precision, RoundingMode.FLOOR).setScale(precision, RoundingMode.FLOOR).stripTrailingZeros();
        sin = ECy.subtract(BCy).divide(L_15, precision, RoundingMode.FLOOR).setScale(precision, RoundingMode.FLOOR).stripTrailingZeros();
