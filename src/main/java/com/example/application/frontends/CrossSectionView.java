...
public class CrossSectionView extends VerticalLayout {
    ...
    private void validateAndSave (MyUI UI) {
        ...
        if (IF.crossSections.size() == InputFrame.crossSectionMaxItem) {
            ...
            ErrorNotification.displayNotification("Non-positive Value Error : The value of cross-sectional area must be positive.");
        }
        else if (E_.compareTo(BigDecimal.ZERO) <= 0) {
            ErrorNotification.displayNotification("Non-positive Value Error : The value of modulus of elasticity must be positive.");
        }
        else if (I_.compareTo(BigDecimal.ZERO) <= 0) {
            ErrorNotification.displayNotification("Non-positive Value Error : The value of moment of inertia must be positive.");
        }
        else {
            boolean add = true;
    ...
    private VerticalLayout configureEditForm (CrossSection CS, MyUI UI) {
        ...
        InputFrame.trim(E_e, 2);
        InputFrame.trim(I_e, 2);
    ...
    private void validateAndEdit (Dialog dialog, CrossSection CS, MyUI UI) {
        ...
        if (A_.compareTo(BigDecimal.ZERO) <= 0) {
            ErrorNotification.displayNotification("Non-positive Value Error : The value of cross-sectional must be positive.");
        }
        else if (E_.compareTo(BigDecimal.ZERO) <= 0) {
            ErrorNotification.displayNotification("Non-positive Value Error : The value of modulus of elasticity must be positive.");
        }
