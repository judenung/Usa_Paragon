...
public class LinearlyDistributedLoadView extends VerticalLayout {
    ...
    private void configureForm (MyUI UI) {
        ...
        InputFrame.trim(W1, 4);
        InputFrame.trim(W2, 4);
        InputFrame.trim(a, 2);
        InputFrame.trim(b, 2);
    ...
    private void validateAndSave (MyUI UI) {
        ...
        else if (a_.compareTo(IF.frameMembers.get(MID.getValue() - 1).L) > 0 || a_.compareTo(BigDecimal.ZERO) < 0 ||
                 b_.compareTo(IF.frameMembers.get(MID.getValue() - 1).L) > 0 || b_.compareTo(BigDecimal.ZERO) < 0) {
            ErrorNotification.displayNotification("Out of Bound Error : Linearly distributed load is located outside of the frame member's span.");
        }
    ...
    private VerticalLayout configureEditForm (LinearlyDistributedLoad LDL, MyUI UI) {
        ...
        InputFrame.trim(W1_e, 4);
        InputFrame.trim(W2_e, 4);
        InputFrame.trim(a_e, 2);
        InputFrame.trim(b_e, 2);
    ...
    private void validateAndEdit (Dialog dialog, LinearlyDistributedLoad LDL, MyUI UI) {
        ...
        else if (a_.compareTo(IF.frameMembers.get(MID_e.getValue() - 1).L) > 0 || a_.compareTo(BigDecimal.ZERO) < 0 ||
                b_.compareTo(IF.frameMembers.get(MID_e.getValue() - 1).L) > 0 || b_.compareTo(BigDecimal.ZERO) < 0) {
            ErrorNotification.displayNotification("Out of Bound Error : Linearly distributed load is located outside of the frame member's span.");
        }
