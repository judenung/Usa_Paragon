...
public class PointLoadView extends VerticalLayout {
    ...
    private void configureForm (MyUI UI) {
        ...
        InputFrame.trim(d, 2);
        InputFrame.trim(Fx, 4);
        InputFrame.trim(Fy, 4);
        InputFrame.trim(Mz, 4);
    ...
    private void validateAndSave (MyUI UI) {
        ...
        else if (d_.compareTo(IF.frameMembers.get(MID.getValue() - 1).L) > 0 || d_.compareTo(BigDecimal.ZERO) < 0) {
            ErrorNotification.displayNotification("Out of Bound Error : Point load is located outside of the frame member's span.");
        }
    ...
    private VerticalLayout configureEditForm (PointLoad PL, MyUI UI) {
        ...
        InputFrame.trim(d_e, 2);
        InputFrame.trim(Fx_e, 4);
        InputFrame.trim(Fy_e, 4);
        InputFrame.trim(Mz_e, 4);
