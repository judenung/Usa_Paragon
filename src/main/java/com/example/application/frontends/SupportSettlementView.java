...
public class SupportSettlementView extends VerticalLayout {
    ...
    private void configureForm (MyUI UI) {
        ...
        SP.addValueChangeListener(event -> {
            Support sp = IF.supports.get(SPs.indexOf(SP.getValue()));
            Joint temp_j = new Joint(BigDecimal.ZERO, BigDecimal.ZERO);
            temp_j.setJoinType(sp.ST);
            Sx.setEnabled(temp_j.DR[0]);
            Sy.setEnabled(temp_j.DR[1]);
            Sz.setEnabled(temp_j.DR[2]);
            if (!temp_j.DR[0]) {
                Sx.clear();
            }
            if (!temp_j.DR[1]) {
        ...
        InputFrame.trim(Sx, 2);
        InputFrame.trim(Sy, 2);
        InputFrame.trim(Sz, 2);
    ...
    private VerticalLayout configureEditForm (SupportSettlement SS, MyUI UI) {
        ...
        SP_e.addValueChangeListener(event -> {
            ...
            Joint temp_j = new Joint(BigDecimal.ZERO, BigDecimal.ZERO);
            temp_j.setJoinType(sp.ST);
            Sx_e.setEnabled(temp_j.DR[0]);
            Sy_e.setEnabled(temp_j.DR[1]);
            Sz_e.setEnabled(temp_j.DR[2]);
            if (!temp_j.DR[0]) {
                Sx_e.clear();
            }
            if (!temp_j.DR[1]) {
        ...
        InputFrame.trim(Sx_e, 2);
        InputFrame.trim(Sy_e, 2);
        InputFrame.trim(Sz_e, 2);
        ...
        int i = 0;
        for (Joint j : IF.joints) {
            if (j.Cx.compareTo(SS.Cx) == 0 && j.Cy.compareTo(SS.Cy) == 0) {
                break;
            }
            i++;
        }
