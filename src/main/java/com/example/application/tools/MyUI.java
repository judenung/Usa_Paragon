...
public class MyUI {
    ...
    public void refreshAllGrid () {
        ...
        if (jointLoadView != null && buildView.tabs.getSelectedIndex() == 3) {
            jointLoadView.refresh(this);
        }
        if (pointLoadView != null && buildView.tabs.getSelectedIndex() == 4) {
            pointLoadView.refresh(this);
        }
        if (linearlyDistributedLoadView != null && buildView.tabs.getSelectedIndex() == 5) {
            linearlyDistributedLoadView.refresh(this);
        }
        if (temperatureChangeView != null && buildView.tabs.getSelectedIndex() == 6) {
            temperatureChangeView.refresh(this);
        }
        if (fabricationErrorView != null && buildView.tabs.getSelectedIndex() == 7) {
            fabricationErrorView.refresh(this);
        }
        if (supportSettlementView != null && buildView.tabs.getSelectedIndex() == 8) {
            supportSettlementView.refresh(this);
        }
        if (solutionView != null && buildView.tabs.getSelectedIndex() == 9) {
